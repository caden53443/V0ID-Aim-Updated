...
class Aimbot:
    ...
    ii_ = Input_I()
    screen = mss.mss()
    pixel_increment = 4.00 #controls how many pixels the mouse moves for each relative movement
    ...
    def is_target_locked(x, y):
        ...
        threshold = 5
        return True if 864 - threshold <= x <= 864 + threshold and 530 - threshold <= y <= 540 + threshold else False#if messup was 540
    ...
    def interpolate_coordinates_from_center(absolute_coordinates, scale):
        diff_x = (absolute_coordinates[0] - 864) * scale/Aimbot.pixel_increment
